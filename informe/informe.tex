...
\vspace{0.3cm}
\begin{tabular}{ll}
\textcolor{ccsgray}{Balcazar Santa Cruz, Kiara Alexandra} & \textcolor{ccsgray}{[202210403]} \\
\textcolor{ccsgray}{Gonzales Torres, Aaron Naif} & \textcolor{ccsgray}{[202XXXXXX]} \\
\textcolor{ccsgray}{Surco Vergara, Maria Fernanda} & \textcolor{ccsgray}{[202110358]} \\
\end{tabular}
...
\paragraph{¿Por qué es importante resolverlo?} La climatización representa una
fracción considerable del gasto eléctrico de un campus. Ajustar la demanda a la
...
La telemetría se captura con sensores de bajo costo sobre un microcontrolador
que la envía al backend por HTTP. En la versión actual se cuenta con un nodo
sensor funcional; el resto del despliegue se modela mediante un simulador y un
dataset sembrado reproducible (ver §\ref{sec:viab}).
...
\midrule
DHT22 & Ambiente térmico & Temperatura (°C), humedad (\%) & \textasciitilde 30 s \\
MH-Z19B & Calidad de aire & CO\textsubscript{2} (ppm) $\rightarrow$ ocupación real & \textasciitilde 30 s \\
MQ-7 & Seguridad & Monóxido de carbono CO (ppm) & \textasciitilde 30 s \\
Emisor IR & Actuador & Comando de \emph{setpoint} al A/C & bajo demanda \\
ESP32 / Raspberry Pi & Gateway & Agrega y transmite las lecturas & --- \\
\bottomrule
\end{tabular}
\caption{Hardware, función, datos y frecuencia de envío.}
\end{table}

\paragraph{Conexión con la aplicación.} El gateway (ESP32/Raspberry Pi) publica
cada lectura como \texttt{POST /api/v1/sensors/} en formato JSON
(\texttt{sensor\_id}, \texttt{temperature}, \texttt{humidity},
\texttt{co2\_ppm}, \texttt{co\_ppm}, \texttt{timestamp}), autenticado por
\texttt{X-API-Key}. El backend persiste la lectura y ejecuta la acción
cognitiva; el actuador IR recibe de vuelta el \emph{setpoint} sugerido. El
...
% hardware
\node[hw,below=16mm of app](hw){Sensores + ESP32\\DHT22 · MH-Z19 · MQ-7};
...
\draw[arr](api)--(pg);
\draw[arr](api)--(ml);
\draw[arr](app.south)|-(watson.west);
...
el backend FastAPI; éste persiste en MongoDB/PostgreSQL, invoca el modelo ML e
IBM Watson, y recibe la telemetría del hardware IoT.}
\end{figure}
...
IA & scikit-learn (\texttt{HistGradientBoostingRegressor}), IBM Watson Assistant \\
...
\begin{itemize}
\item Despliegue en la nube (AWS/GCP).
...
\section{Recursos y enlaces del proyecto}
