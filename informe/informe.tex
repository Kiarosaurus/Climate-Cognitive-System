...
% Digital-first link styling on the project palette (URLs must be visibly links).
\hypersetup{colorlinks=true, urlcolor=ccsblue, linkcolor=ccsdark, citecolor=ccsgray}

% Compact bibliography spacing so the reference list closes with the document
% instead of orphaning the last entry onto a near-empty page.
\let\oldthebibliography\thebibliography
\renewcommand{\thebibliography}[1]{\oldthebibliography{#1}\setlength{\itemsep}{2pt}}
...
real, temperatura exterior y metadata física del aula. Se valida con
\emph{holdout} cronológico frente a dos \emph{baselines} (media y heurística de
ocupación); el modelo solo se despliega si las supera
(\texttt{beats\_baselines}).
...
\begin{itemize}
\item Migración a la nube (AWS/GCP).
...
% ============================ REFERENCIAS =================================
% Dedicated final page: without the break the list splits and strands the last
% entry alone on a near-empty page.
\newpage
