...
\begin{frame}{Aplicación web y hardware}
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{Hardware (ESP32 / RPi)}
      \begin{itemize}\small
        \item DHT22 --- temperatura / humedad
        \item MH-Z19B --- CO\textsubscript{2} $\rightarrow$ ocupación
        \item MQ-7 --- monóxido de carbono
        \item Emisor IR --- control del A/C
      \end{itemize}
      \footnotesize Envío \texttt{POST /sensors/} cada ${\sim}30$\,s.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Arquitectura de la solución}
  ...
  \begin{tikzpicture}[
    ...
    \node[hw,below=12mm of app](hw){Sensores + ESP32};
...
\begin{frame}{Tecnologías}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    IA       & scikit-learn, IBM Watson Assistant \\
    Hardware & ESP32 / Raspberry Pi, DHT22, MH-Z19B, MQ-7, IR \\
